...
namespace DemoWin
{
    public partial class FRegister : Form
    {
        ...
        private void btnRegister_Click(object sender, EventArgs e)
        {
            ...
            if(person.Role == "Nguoi dùng")
                if (modify.Accounts("Select * from NguoiDung where Email = '" + email + "'").Count != 0) { MessageBox.Show("Email này đã được sử dụng vui lòng dùng Email khác !!!"); return; }
            else
                if (modify.Accounts("Select * from Worker where Email = '" + email + "'").Count != 0) { MessageBox.Show("Email này đã được sử dụng vui lòng dùng Email khác !!!"); return; }
            try
            {
                if (person.Role == "Người dùng")
                {    
                    string query = string.Format("Insert into NguoiDung(ID , Ten, Email, TenTaiKhoan, MatKhau, VaiTro) VALUES ('{0}', '{1}', '{2}','{3}','{4}','{5}')", person.Id, person.Name, person.Email, person.Account, person.Pass, person.Role);
                ...
                {
                    string query = string.Format("Insert into Worker(ID , Ten, Email, TenTaiKhoan, MatKhau, VaiTro) VALUES ('{0}', '{1}', '{2}','{3}','{4}','{5}')", person.Id, person.Name, person.Email, person.Account, person.Pass, person.Role);
                    modify.Command(query);
                    if (MessageBox.Show("Đăng ký thành công! Bạn có muốn đăng nhập luôn không?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
                    {
                        this.Close();
